...
class Graph:
    ...
    def find_all_paths_dfs(self, start_node, end_node):
        graph = nx.DiGraph()
        graph.add_edges_from(self.edges)
...
# Function to create a random attack graph
def create_attack_graph(num_nodes=30):
    graph = Graph()
...
start_time = time.perf_counter()
dfs_paths = attack_graph.find_all_paths_dfs(src_node, dest_node)
dfs_time = time.perf_counter() - start_time
print(f'DFS Time: {dfs_time * 1000:.4f} milliseconds')  # Convert to milliseconds

print(f'Number of BFS Paths: {len(bfs_paths)}')
print(f'Number of DFS Paths: {len(dfs_paths)}')

# print(f'\nBFS Paths: {bfs_paths}')
# print(f'DFS Paths: {dfs_paths}')
